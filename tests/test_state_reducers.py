...
class TestEnvelopeReducer:
    def test_empty_existing(self):
        ...
        new = [{"task_id": "a", "worker": "w1", "status": "ok"}]
        assert envelope_reducer([], new) == new

    def test_upsert_by_task_id(self):
        """An update to an existing task_id merges in place, not duplicate."""
        existing = [{"task_id": "a", "worker": "w1", "status": "ok", "persisted": False}]
        new = [{"task_id": "a", "worker": "w1", "status": "ok", "persisted": True}]
    ...
    def test_later_entries_win_field_wise(self):
        ...
        existing = [{"task_id": "a", "worker": "w1", "status": "ok", "summary": "old"}]
        new = [{"task_id": "a", "worker": "w1", "status": "ok", "summary": "new"}]
        result = envelope_reducer(existing, new)
    ...
    def test_preserves_structured_when_update_omits_it(self):
        ...
        existing = [
            {"task_id": "a", "worker": "w1", "structured": {"x": 1}, "persisted": False}
        ]
        new = [{"task_id": "a", "worker": "w1", "persisted": True}]
        result = envelope_reducer(existing, new)
    ...
    def test_explicit_structured_none_is_kept(self):
        ...
        new = [{"task_id": "a", "worker": "w1", "structured": None, "persisted": True}]
        result = envelope_reducer(existing, new)
        assert result[0]["structured"] is None
        assert result[0]["persisted"] is True

    def test_distinct_task_ids_accumulate(self):
        """Distinct task_ids accumulate in first-seen order."""
        existing = [{"task_id": "a", "worker": "w1"}]
        new = [{"task_id": "b", "worker": "w2"}]
        result = envelope_reducer(existing, new)
        assert [e["task_id"] for e in result] == ["a", "b"]

    def test_fallback_key_worker_as_of(self):
        """Entries without task_id fall back to worker:as_of."""
    ...
    def test_insertion_order_preserved(self):
        """Order is insertion order of first-seen task_ids."""
        existing = [{"task_id": "a"}, {"task_id": "b"}]
        new = [{"task_id": "c"}, {"task_id": "a", "status": "updated"}]
        result = envelope_reducer(existing, new)
        assert [e["task_id"] for e in result] == ["a", "b", "c"]
